...
1. Create a repo (duh).  My dev-ops process is very simple. Create a local clone.  Host locally with ```python -m http.server```.  Push updates on a nightly basis, using a .gitignore to prevent pusinging dev artifacts.
...
3. Create a AWS workflow to push your github repo change automatically to S3.  You can use the workflow in this repo as an example.  In your repo settings you create two secrets (AWS_ACCESS_KEY and AWS_SECRET_ACCESS_KEY) using your AWS credentials.
4. 
